...
f"""
        ...
        WHERE 
            ...
            (
                SELECT DISTINCT order_id FROM delta.`s3://ecommerce-project-trusted/ecommerce/olist_order_items_dataset/`
            )
"""
)
...
spark.sql(
f"""
    INSERT INTO delta.`s3://ecommerce-project-trusted/ecommerce/olist_order_items_dataset/`
    (
        SELECT
            ref_day,
            ref_file_extraction,
            ...
            cast(freight_value as float) nu_freight_value
        FROM raw_order_items as raw
        WHERE 
            ref_day_partition = {__REFDAY__}
            and raw.order_id NOT IN

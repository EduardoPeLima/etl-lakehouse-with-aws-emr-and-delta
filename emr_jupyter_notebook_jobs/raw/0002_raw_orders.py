...
str_landzone_file_path = "ecommerce/olist_orders_dataset/"

str_raw_file_path = "ecommerce/olist_orders_dataset/"

str_s3_landzone_file_path = f's3://{str_bucket_landzone}/{str_landzone_file_path}'
print(str_s3_landzone_file_path)

landzone_orders = spark.read.format(
...
landzone_orders.createOrReplaceTempView("landzone_orders")

raw_orders = spark.sql(
    f"""
        SELECT
...
control = spark.sql(
    f"""
        SELECT
            ...
            '0002_raw_orders' as str_process, 

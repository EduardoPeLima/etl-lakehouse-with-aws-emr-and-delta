...
spark = SparkSession.builder \
    .appName('0004_raw_order_payments') \
...
str_landzone_file_path = "ecommerce/olist_order_payments_dataset/"

str_raw_file_path = "ecommerce/olist_order_payments_dataset"

str_s3_landzone_file_path = f's3://{str_bucket_landzone}/{str_landzone_file_path}'
print(str_s3_landzone_file_path)

landzone_order_payments = spark.read.format(
...
landzone_order_payments.createOrReplaceTempView("landzone_order_payments")

raw_order_payments = spark.sql(
    f"""
        SELECT
            ...
            --file fields
            string(order_id),
            string(payment_sequential),
            string(payment_type),
            string(payment_installments),
            string(payment_value)
        FROM landzone_order_payments
    """
)
...
control = spark.sql(
    f"""
        SELECT
            ...
            '0004_raw_order_payments' as str_process, 
